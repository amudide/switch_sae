...
wandb.init(entity="amudide", project="ReLU (Feb)", config={f'{trainer_config["wandb_name"]}-{i}' : trainer_config for i, trainer_config in enumerate(trainer_configs)})
...
for i, trainer_config in enumerate(trainer_configs):
    ae = AutoEncoder.from_pretrained(f'dictionaries/{cfg_filename(trainer_config)}/ae.pt', device=device)
